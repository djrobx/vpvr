...
template<class T> int   Contains(const eastl::vector<T> & c, const T & t){ return (int)std::count(begin(c), end(c), t); }
template<class T> int   IndexOf(const eastl::vector<T> & c, const T & v) { return (int)(std::find(begin(c), end(c), v) - begin(c)); }
//template<class T> T &   Add(eastl::vector<T> & c, T t)                   { c.push_back(t); return c.back(); }
template<class T> T     Pop(eastl::vector<T> & c)                        { const T val = std::move(c.back()); c.pop_back(); return val; }
template<class T> void  AddUnique(eastl::vector<T> & c, T t)             { if (!Contains(c, t)) c.push_back(t); }
//template<class T> void  Remove(eastl::vector<T> & c, T t)                { const eastl::vector<T>::const_iterator it = std::find(begin(c), end(c), t); assert(it != end(c)); c.erase(it); assert(!Contains(c, t)); }
template<class T> void  RemoveFillWithBack(eastl::vector<T> & c, T t)    { const int idxof = IndexOf(c, t); const T val = Pop(c); if (idxof == c.size()) return; c[idxof] = val; assert(!Contains(c, t)); }
...
public:
	...
	eastl::vector<Vertex *>   neighbor; // adjacent vertices
	eastl::vector<Triangle *> face;     // adjacent triangles
...
static eastl::vector<Vertex *>   vertices;
static eastl::vector<Triangle *> triangles;
...
__forceinline void Collapse(Vertex * const u, Vertex * const v)
{
	...
	eastl::vector<Vertex *> tmp(u->neighbor.size());
...
__forceinline void AddVertex(const eastl::vector<float3> &vert)
{
	for (size_t i = 0; i < vert.size(); i++)
		Vertex *v = new Vertex(vert[i], i); //!! braindead design, actually fills up "vertices"
}

__forceinline void AddFaces(const eastl::vector<tridata> &tri)
{
...
void ProgressiveMesh(const eastl::vector<float3> &vert, const eastl::vector<tridata> &tri,
					 eastl::vector<unsigned int> &map, eastl::vector<unsigned int> &permutation)
{
...
__forceinline unsigned int MapVertex(unsigned int a, const unsigned int mx, const eastl::vector<unsigned int> &map)
{
	while (a >= mx)
		a = map[a];
	return a;
...
void ReMapIndices(const unsigned int num_vertices, const eastl::vector<tridata> &tri, eastl::vector<tridata> &new_tri, const eastl::vector<unsigned int> &map)
{
