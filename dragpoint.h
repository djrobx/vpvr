...
public:
   ...
   void GetTextureCoords(const eastl::vector<RenderVertex> & vv, float **ppcoords);
...
protected:
   template <typename T>
   void GetRgVertex(eastl::vector<T> &vv, const bool loop = true, const float accuracy = 4.f) const // 4 = maximum precision that we allow for
   {
