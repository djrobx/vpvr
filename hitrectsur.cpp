...
void HitRectSur::Polygon(const eastl::vector<RenderVertex> &rgv)
{
   if (m_fFailedAlready)
   {
      return;
...
void HitRectSur::PolygonImage(const eastl::vector<RenderVertex> &rgv, HBITMAP hbm, const float left, const float top, const float right, const float bottom, const int bitmapwidth, const int bitmapheight)
{
   Polygon(rgv);
}
